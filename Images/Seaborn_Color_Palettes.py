...
color_list = ['Accent', 'Accent_r', 'Blues', 'Blues_r', 'BrBG', 'BrBG_r', 'BuGn', 'BuGn_r', 'BuPu', 'BuPu_r',\
...
'Purples', 'Purples_r', 'RdBu', 'RdBu_r', 'RdGy', 'RdGy_r', 'RdPu', 'Set3_r', 'Spectral', 'Spectral_r',\
...
# color_dict = {color_dict[x] == color_list[x] for x in range(len(color_list))}

color_dict ={}
for color in range(len(color_list)):
    color_dict[color] = color_list[color]
...
def show_all_dict():
    ...
    for color in color_dict:
        print((str(color) + ":  " + color_dict[color]), end=',  ')
        if ctr % 5 == 0:
            print()
        ctr += 1
